...
export interface NodeRecord {
  ...
  status: "Optimal" | "Infeasible" | "Pruned (Bound)" | "Integer Found" | "Branched"
  solution: string // Texto legível: "[2.5, 3.0]"
}

export class BranchAndBoundSolver {
  private bestObjective: number = Number.POSITIVE_INFINITY // Minimização: começa com infinito
  ...
  public solve(): OptimizationResult {
    ...
    // 1. Inicializa a pilha (DFS) com o nó raiz (sem restrições extras)
    ...
    while (stack.length > 0) {
      ...
      // 2. Resolve a Relaxação Linear deste nó
      ...
      if (result.status === "Infeasible" || result.status === "NotDualFeasible") {
      ...
      record.objectiveValue = Number.parseFloat(result.objectiveValue.toFixed(4))
      record.solution = `[${result.solution.map((n) => Number(n.toFixed(2))).join(", ")}]`
   ...
   */
  private getBranchingCandidates(solution: number[]): { index: number; value: number }[] {
    const candidates = []
    const EPSILON = 1e-5
