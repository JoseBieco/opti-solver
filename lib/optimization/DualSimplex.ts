import type { OptimizationResult, SolutionStatus } from "./types"
...
export class DualSimplexSolver {
   ...
   */
  constructor(c: number[], A: number[][], b: number[], maxIterations = 1000) {
  ...
  private buildTableau(c: number[], A: number[][], b: number[]) {
    ...
    this.tableau = new Array(this.numRows).fill(0).map(() => new Array(this.numCols).fill(0))
  ...
  //     return {
  //       status: "NotDualFeasible",
  //       objectiveValue: 0,
  //       solution: [],
  //       iterations: 0,
  //       message:
  //         "Os custos reduzidos iniciais contêm valores negativos. O Dual Simplex requer otimalidade inicial (c >= 0).",
  //     };
  //   }
  ...
  //     // Passo 1: Encontrar a variável que sai da base (Linha Pivô)
  //     // No Dual Simplex, buscamos o RHS mais negativo.
  //     const pivotRow = this.getLeavingVariable();

  //     // Se pivotRow for -1, todos RHS >= 0. Solução é Viável e Ótima.
  //     if (pivotRow === -1) {
  //       return this.extractSolution(iterations, "Optimal");
  //     }

  //     // Passo 2: Encontrar a variável que entra na base (Coluna Pivô)
  //     const pivotCol = this.getEnteringVariable(pivotRow);

  //     // Se não houver candidato para entrar, o problema é inviável (dual unbounded)
  //     if (pivotCol === -1) {
  //       return this.extractSolution(iterations, "Infeasible");
  //     }

  //     // Passo 3: Realizar o Pivoteamento Gaussiano
  //     this.pivot(pivotRow, pivotCol);
  //     iterations++;
  //   }

  //   return this.extractSolution(iterations, "MaxIterations"); // Ou limite de iterações atingido
  // }
  ...
  private runPrimalSimplex(): SolutionStatus {
    ...
    while (iterations < maxIterations) {
      // 1. Escolher quem ENTRA (Coluna com custo mais negativo)
      ...
      // 2. Escolher quem SAI (Menor Razão Primal: b / a para a > 0)
  ...
  /**
   * Executa o algoritmo Dual Simplex (sua lógica original).
   * Nota: Renomeei seu antigo loop 'solve' para este nome para organizar.
   */
  private runDualSimplex(): OptimizationResult {
    let iterations = 0

    while (iterations < this.maxIterations) {
      // 1. Escolher quem SAI (Linha com RHS mais negativo)
      ...
      // 2. Escolher quem ENTRA (Teste da Razão Dual)
  ...
  private pivot(row: number, col: number) {
    ...
    // 1. Normalizar a linha pivô (dividir tudo pelo elemento pivô)
    for (let j = 0; j < this.numCols; j++) {
      this.tableau[row][j] /= pivotElement
    }

    // 2. Zerar a coluna pivô nas outras linhas
  ...
  private extractSolution(iterations: number, status: SolutionStatus): OptimizationResult {
